...
def draw_menu():
    ...
    window.blit(play_text, (menu_play_button_rect.centerx - play_text.get_width() // 2,
                            menu_play_button_rect.centery - play_text.get_height() // 2))
    pygame.display.flip()
...
def draw_instructions():
    ...
    lines = [
        ...
        "Reach the end of each level before fuel runs out!",
        "Click PLAY to begin!"
    ]
...
def show_level_summary(level, score):
    window.fill(white)
    level_text = title_font.render(f"Level {level} Complete!", True, black)
    score_text = score_font.render(f"Score: {score}", True, black)
    window.blit(level_text, (screen_width // 2 - level_text.get_width() // 2, 180))
    window.blit(score_text, (screen_width // 2 - score_text.get_width() // 2, 260))
    pygame.display.flip()
...
def display_score(score, high_score, fuel_percent, level):
...
def run_game():
    ...
    for level in range(1, 4):  # Three levels
        ...
        while running:
            ...
            # Plastic spawning and behavior
            if random.randint(1, 100) <= plastic_chance[level]:
            ...
            # Fish spawning (throttled by frame count)
            if frame_counter % 5 == 0:
                if random.randint(1, 100) <= fish_chance[level]:
            ...
            if fuel_left <= 0:
                ...
                if level < 3:
                    show_level_summary(level, current_score)
                else:
                    draw_game_over(current_score)
                running = False

            # Score and HUD
            display_score(current_score, high_score, fuel_left, level)
            pygame.display.flip()
            clock.tick(100)
